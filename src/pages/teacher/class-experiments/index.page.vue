<template>
  <div class="p-1">
        ...
        <div class="flex flex-wrap gap-4 items-center">
          ...
          <div class="flex-1 min-w-50">
            <Select v-model="selectedCourseId" :options="courseOptions" option-label="label" option-value="value"
              placeholder="选择课程" class="w-full" filter show-clear />
          </div>

          <!-- 班级筛选 -->
          <div class="flex-1 min-w-50">
            <Select v-model="selectedClassCode" :options="classOptions" option-label="label" option-value="value"
              placeholder="选择班级" class="w-full" filter show-clear />
          </div>

          <!-- 实验筛选 -->
          <div class="flex-1 min-w-50">
            <Select v-model="selectedExperimentId" :options="experimentOptions" option-label="label"
              option-value="value" placeholder="选择实验" class="w-full" filter show-clear />
          </div>
        </div>
      </template>
    </Card>
    ...
    <Card>
      <template #content>
        <DataTable v-model:selection="selectedClassExperiments" :value="filteredClassExperiments" :loading="isLoading"
          selection-mode="multiple" :paginator="true" :rows="10" :pt="{ header: { class: 'px-0!' } }">
          <template #header>
          ...
          <Column header="操作">
            <template #body="slotProps">
              <div class="flex gap-2">
                <Button label="签到管理" outlined size="small" @click="openAttendanceManagement(slotProps.data)" />
                <Button label="学生批改" outlined size="small" @click="openStudentGrading(slotProps.data)" />
                <Button label="统计信息" outlined severity="secondary" size="small"
...
import { BindClassExperimentDialog, useQueryClassExperiments } from '@/features/teacher/class-experiment'
import { AttendanceManagementDialog, StudentGradingDialog, ClassExperimentStatisticsDialog } from '@/features/teacher/class-experiment'
// ==================== 类型定义 ====================
