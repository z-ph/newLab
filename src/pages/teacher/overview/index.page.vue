        ...
        <template #content>
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div
                v-else-if="courses?.records?.length"
                v-for="course in courses.records"
                :key="course.id"
        ...
        <template #content>
          <div class="space-y-4">
            <div class="flex items-center justify-between">
              ...
              <Button
                v-if="experiments && experiments.total > 5"
                label="查看更多"
                size="small"
                text
            ...
            <div class="space-y-2">
              <div v-if="isLoading" class="text-sm text-slate-400">加载中...</div>
              <div
                v-else-if="experiments?.records?.length"
                v-for="experiment in experiments.records"
                :key="experiment.id"
                class="flex items-center justify-between rounded-lg border border-slate-100 p-2 hover:bg-slate-50"
              >
        ...
        <template #content>
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              <div v-if="isLoading" class="text-sm text-slate-400">加载中...</div>
              <div
                v-else-if="classes?.records?.length"
                v-for="cls in classes.records"
                :key="cls.classCode"
                class="flex items-center justify-between rounded-lg border border-slate-100 p-2 hover:bg-slate-50"
              >
...
import { useOverviewStatistics, ClassCourseExperimentsImportDialog } from '@/features/teacher/overview'
